...
file_path = 'page_india_delhi_roc_ageB.txt'  # File path to save the elements

max_retries_per_page = 5  # Maximum number of retries for a page
sleep_duration_on_success = 2  # Duration to sleep after each successful response (in seconds)
page_count = 1753
        ...
        while current_retries < max_retries_per_page and not success:
            try:
                if not proxies:
                    ...
                    url = f"https://www.zaubacorp.com/company-list/age-B/roc-RoC-Delhi/p-{page}-company.html"
